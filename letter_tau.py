...
for i in range(iters3):
    ...
    comm_ADA_Wireless_modified2[i] += len(selection) * 2
...
A = 1
iters3 = 60000
t_ADA_Wireless_modified = [0]*(iters3+1)
comm_ADA_Wireless_modified = [0]*(iters3+1)
...
for i in range(iters3):
    ...
    if i % 10 == 0:
        ...
        test_losses_ADA_Wireless_modified.append(100. * correct_ADA_Wireless_modified / len(test_loader.dataset))
    ...
    comm_ADA_Wireless_modified[i] += len(selection) * 2
    comm_ADA_Wireless_modified[i+1] = comm_ADA_Wireless_modified[i]   
    if test_losses_ADA_Wireless_modified[-1] >= acc:
        break
...
for i in range(iters3):
    ...
    comm_ADA_Wireless_modified4[i] += len(selection) * 2
...
figure()
plot(np.arange(0, len(test_losses_ADA_Wireless_modified3)*10, 10), test_losses_ADA_Wireless_modified3[0::1],linestyle='--',color='black')
...
legend([r"$\tau_{max}=1$",r"$\tau_{max}=7000$",r"$\tau_{max}=4$"])

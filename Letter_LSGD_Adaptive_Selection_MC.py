...
#                             split = typeMNIST
#                             )
dataset_global = torchvision.datasets.FashionMNIST(
                            root='./data',
                            train=True,
...
def poc(seq, prob, S):
    ...
    while True:
        ...
        if len(out) == S:
            break
    return out
# p = [50,10,40,20,30,30,50,30,40,30,40,40,20,50,30,20,10,30,10,5]
...
for i in range(M):
    ...
    #                         split = typeMNIST
    #                         ))
    dataset_workers.append(torchvision.datasets.FashionMNIST(
                            root='./data',
                            train=True,
                            transform=transform1,
                            download = False
...
# xlim([0, 30000])
grid('on')
savefig('comm_accuracy_state_of_the_art_letter.pdf')
...
# xlim([0, 30000])
grid('on')
savefig('iter_accuracy_state_of_the_art_letter.pdf')
